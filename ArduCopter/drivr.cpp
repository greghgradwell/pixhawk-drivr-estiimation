...
void Copter::setup_telem()
{
    // hal.scheduler->delay(1000);
    setup_uart(hal.serial(2), 230400);
    _drivr_port_ready = true;
}

void Copter::send_drivr_message(void)
{
    if (_drivr_port_ready)
    {
        Vector3f attitude;
        ...
        uint8_t fix = static_cast<uint8_t>(copter.gps.status(1));
        // if (fix > 0)
        // {
